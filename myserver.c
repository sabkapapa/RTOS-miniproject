...
{ 
	...
	/* a new playback stream is to be created */
	...
	if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) return -1; //  file descriptor of socket
	if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,&opt, sizeof(opt))) return -1; // attaching
